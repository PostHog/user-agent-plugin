function _includes<T = any>(str: T[] | string, needle: T): boolean {
    return (str as any).indexOf(needle) !== -1
}

function _isFunction(x: unknown): x is Function {
    return typeof x === 'function'
}

export function _isUndefined(x: unknown): x is undefined {
    return typeof x === 'undefined'
}

/**
 * This code was copied unchanged from posthog-js as it should match that libraries behavior exactly
 *
 *  --------
 *
 * this device detection code was (at time of writing) about 3% of the size of the entire posthog-js library
